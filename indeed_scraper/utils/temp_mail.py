...
class EmailProvider(abc.ABC):
    ...
    @abc.abstractmethod
    def wait_for_otp(self, token: str, timeout_seconds: int = 180, poll_interval: int = 5) -> str | None: ...
...
class MailTmProvider(EmailProvider):
    ...
    def create_account(self) -> tuple[str, str]:
        res = self._session.get(f"{self.BASE_URL}/domains")
...
class GuerrillaMailProvider(EmailProvider):
    ...
    def wait_for_otp(self, sid_token: str, timeout_seconds: int = 180, poll_interval: int = 5) -> str | None:
        start = time.time()
        seq = 0
        while time.time() - start < timeout_seconds:
            try:
...
class TempMailManager:
    ...
    def create_new_email(self, skip_providers: list[str] | None = None) -> tuple[str, str]:
        for provider in self._providers:
            if skip_providers and type(provider).__name__ in skip_providers:
                continue
            try:
                email, raw_token = provider.create_account()
    ...
    def wait_for_otp_code(self, encoded_token: str, timeout_seconds: int = 180, poll_interval: int = 5) -> str | None:
        name, raw_token = encoded_token.split(":", 1)
        for provider in self._providers:
            if type(provider).__name__ == name:
                return provider.wait_for_otp(raw_token, timeout_seconds, poll_interval)
